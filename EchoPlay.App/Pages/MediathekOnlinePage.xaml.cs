...
namespace EchoPlay.App.Pages
{
    ...
    public sealed partial class MediathekOnlinePage : Page
    {
        ...
        /// <summary>
        /// Lädt die Serienliste und registriert Events.
        /// </summary>
        protected override async void OnNavigatedTo(NavigationEventArgs e)
        {
            base.OnNavigatedTo(e);

            // Offline-Modus: Online-Mediathek benötigt Internetverbindung
            using (IServiceScope scope = App.Services.CreateScope())
            {
                ...
                {
                    Services.IErrorDialogService dialogService = App.Services
                        .GetRequiredService<Services.IErrorDialogService>();
        ...
        /// <summary>
        /// Schaltet den Überwachungsstatus einer Serie um.
        /// Überwachte Serien erscheinen im Dashboard unter Neuerscheinungen.
        ...
        private async void OnToggleWatchSeriesClick(object sender, RoutedEventArgs e)
        {
            if (sender is ToggleMenuFlyoutItem item && item.Tag is Guid seriesId)
            {
                ...
                await seriesService.SetWatchedAsync(seriesId, newState);

                // ViewModel-Property aktualisieren
                SeriesCardViewModel? card = ViewModel.Series
                    .FirstOrDefault(c => c.Id == seriesId);
                if (card is not null)
                {
                    card.IsWatched = newState;
                }

                // Bei Aktivierung: Neuerscheinungen für diese Serie sofort prüfen und cachen.
                // Bei Deaktivierung: Cache-Einträge dieser Serie entfernen.
                if (newState)
                {
                    Data.Entities.Library.Series? series = await seriesService.GetByIdAsync(seriesId);
                ...
                {
                    ICachedNewReleaseDataService cacheService = scope.ServiceProvider
